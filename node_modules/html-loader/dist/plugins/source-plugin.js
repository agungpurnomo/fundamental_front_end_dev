...
var _default = options => function process(html, result) {
  ...
  const getAttribute = (tag, attribute, attributes, resourcePath) => {
    return attributeList.find(element => element.tag.toLowerCase() === tag.toLowerCase() && element.attribute.toLowerCase() === attribute.toLowerCase() && (element.filter ? element.filter(tag, attribute, attributes, resourcePath) : true));
  };
  ...
  for (const source of sources) {
    ...
    html = html.substr(0, startIndex + offset) + replacerName + html.substr(startIndex + value.length + offset);
